...
def test_worker_crawl_indexes_real_semantic_corpus_and_answers_groundedly(tmp_path: Path, monkeypatch) -> None:
    try:
        ...
        build_embedding_provider("sentence-transformers")
...
def test_worker_crawl_indexes_hash_corpus_and_answers_groundedly(tmp_path: Path, monkeypatch) -> None:
    ...
    assert database.knowledge_count(crawl_id) >= 2
    res = answer_question(crawl_id, "How long is the refund window for eligible customers?", main._search_knowledge)
    assert res["grounded"] is True
    assert "fourteen days" in res["answer"]
    assert res["citations"]
    assert res["citations"][0]["url"] == "https://owned.example/"
