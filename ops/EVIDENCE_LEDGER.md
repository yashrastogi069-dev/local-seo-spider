...
| **EVID-PIPE-001** | Embedding outages and failures never corrupt or destroy successful crawl data (100% durability). | `tests/test_pipeline_decoupling.py` | 11 pipeline isolation test cases | Provider outage & crash injection before, during, and after indexing | **11/11 tests pass**; 100% pages, links, and lexical chunks preserved | **STRONGLY SUPPORTED** | Two-stage decoupled commit ensures lexical storage commits before vector API calls. |
| **EVID-PIPE-002** | Content hash deduplication skips redundant embedding calls for identical text chunks. | `tests/test_pipeline_decoupling.py` | Controlled test chunks with matching Blake2b hash | Content-hash cache verification | **100% skip rate on matching (hash, provider, model, dim)** | **STRONGLY SUPPORTED** | Changing model or dimension invalidates hash cache and forces re-embedding. |
| **EVID-PIPE-003** | Model mismatch detection strictly prevents cross-model vector contamination. | `tests/test_pipeline_decoupling.py` | Multi-generation model swap fixtures | `detect_embedding_generation_mismatch` and re-index suite | **100% detection rate**; mismatched vectors isolated | **STRONGLY SUPPORTED** | Requires explicit re-indexing when switching models. |
